# `movie-finder` — the seed for AngularJS apps
...
To get you started you can simply clone the `movie-finder` repository and install the dependencies:

### Prerequisites

You need git to clone the `movie-finder` repository. You can get git from [here][git].

We also use a number of Node.js tools to initialize and test `movie-finder`. You must have Node.js
and its package manager (npm) installed. You can get them from [here][node].

### Clone `movie-finder`

Clone the `movie-finder` repository using git:

```
git clone https://github.com/letsbelopez/movie-finder.git
cd movie-finder
```

If you just want to start a new project without the `movie-finder` commit history then you can do:

```
git clone --depth=1 https://github.com/letsbelopez/movie-finder.git <your-project-name>
```
...
The `movie-finder` project supports loading the framework and application scripts asynchronously.
...
The `movie-finder` project comes preconfigured with a local development web server. It is a Node.js
...
your repository and execute scripts such as building the app or running tests. The `movie-finder`
